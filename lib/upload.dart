...
import 'package:awespot_project/map_location.dart';
import 'package:awespot_project/widget/change_hexcolor.dart';
import 'package:path/path.dart' as Path;
import 'package:carousel_pro/carousel_pro.dart';
import 'package:google_place/google_place.dart' as search;
...
class _UploadPageState extends State<UploadPage> {
  ...
  List<String> categories = ["카테고리 미분류", "새내기들과의 부산여행", "2020 나의 첫 배낭여행"];

  String updatedLocation = "위치 정보 없음";
  double updatedLat = 0.0;
  double updatedLng = 0.0;
  ...
  void updateInformation(search.DetailsResult info) {
    setState(() {
      updatedLng = info.geometry.location.lng;
      updatedLat = info.geometry.location.lat;
      updatedLocation = info.name;
    });
  }

  Future relocationMap() async {
    final information = await Navigator.push(
      context,
      MaterialPageRoute(
          fullscreenDialog: true, builder: (context) => MapLocation()),
    );
    return information;
  }
  ...
  Widget showGPS() {
    for (int i = 0; i < gpsLongtitude.length; i++) {
      if (gpsLongtitude[i] != null) {
        updatedLng = gpsLongtitude[i];
        updatedLat = gpsLatitude[i];
        break;
      }
    }
    ...
    return Center(
      ...
      child: Container(
        ...
        child: Text(
          // updatedLng != 0.0 && updatedLat != 0.0
          //     ? "위도: ${updatedLat}, 경도: ${updatedLng}"
          //     : updatedLocation,
          updatedLocation,
          style: TextStyle(
            fontSize: 12,
    ...
    return Scaffold(
            ...
            children: [
              ...
              Row(
                children: [
                  Container(
                    padding:
                        EdgeInsets.fromLTRB(width * 0.04, 0, width * 0.05, 0),
                    width: width * 0.83,
                    child: Text(
                      "콘텐츠 공개 여부",
                      style: TextStyle(
      ...
      body: ListView(
        children: [
          ...
          Container(
            padding: EdgeInsets.fromLTRB(
                query.size.width * 0.1, 0, query.size.width * 0.1, 0),
            child: TextField(
              controller: _titleTextConroller,
              decoration: InputDecoration(
                  border: InputBorder.none,
                  hintText: "제목을 입력하세요.",
                  hintStyle: TextStyle(color: HexColor("#9A9A9A"))),
            ),
          ), //TODO: 제목을 입력하세요
          Container(
            child: Divider(
